/* ============================================================
   Budget Pro — shared app logic (multi-page)
   Data lives in Supabase (Postgres + Auth). See js/config.js for setup.
   ============================================================ */

/* ===================== Brand mark (used in sidebar, topbar, splash, favicon) ===================== */
const LOGO_MARK = '<svg viewBox="0 0 24 24"><path d="M4.5 15.5l4.2-4.2 3.3 3.3L18.5 7" fill="none" stroke="white" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 7h4.5v4.5" fill="none" stroke="white" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
const ICONS = {
  ...
  "Fuel": '<path d="M5 21V6a2 2 0 0 1 2-2h5a2 2 0 0 1 2 2v15"/><path d="M4 21h11"/><path d="M14 9h2a2 2 0 0 1 2 2v4a1.5 1.5 0 0 0 3 0V8l-3-3"/>',
  ...
  "Fitness": '<path d="M4 9v6M20 9v6M2 12h2M20 12h2M7 7v10M17 7v10M7 12h10"/>',
  "Personal Care": '<path d="M12 3v4M12 17v4M4.9 4.9l2.8 2.8M16.3 16.3l2.8 2.8M3 12h4M17 12h4M4.9 19.1l2.8-2.8M16.3 7.7l2.8-2.8"/>',
  "Rent": '<path d="M4 11l8-7 8 7"/><path d="M6 10v10h12V10"/>',
  "Home & Maintenance": '<path d="M14.7 6.3a4 4 0 1 0-5.4 5.4L4 17l3 3 5.3-5.3a4 4 0 0 0 5.4-5.4l-2.3 2.3-2-2 2.3-2.3z"/>',
  "Subscriptions": '<path d="M4 4v5h5M20 20v-5h-5"/><path d="M20 9a8 8 0 0 0-14.3-4.3M4 15a8 8 0 0 0 14.3 4.3"/>',
  "Insurance": '<path d="M12 3l7 3v6c0 4.5-3 8-7 9-4-1-7-4.5-7-9V6l7-3z"/><path d="M9 12l2 2 4-4"/>',
  ...
  "Pets": '<circle cx="7" cy="8" r="1.6"/><circle cx="12" cy="6" r="1.6"/><circle cx="17" cy="8" r="1.6"/><path d="M12 21c-3 0-5-1.6-5-4 0-2 1.7-3 3.2-3.6.6-.2 1-.8 1-1.4 0-.5.3-1 .8-1 .5 0 .8.5.8 1 0 .6.4 1.2 1 1.4C15.3 14 17 15 17 17c0 2.4-2 4-5 4z"/>',
  "Childcare": '<circle cx="12" cy="7" r="3"/><path d="M7 21v-2a5 5 0 0 1 5-5 5 5 0 0 1 5 5v2"/>',
  "Gifts & Donations": '<rect x="3" y="8" width="18" height="13"/><path d="M3 8h18M12 8v13"/><path d="M12 8c-1.5-4-6-4-6 0M12 8c1.5-4 6-4 6 0"/>',
  ...
  "Rental Income": '<path d="M4 11l8-7 8 7"/><path d="M6 10v10h12V10"/><circle cx="12" cy="14" r="1.2"/>',
  "Interest & Dividends": '<circle cx="7" cy="7" r="3"/><circle cx="17" cy="17" r="3"/><path d="M4 20L20 4"/>',
  "Refunds & Cashback": '<path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/><path d="M3 21v-5h5"/>',
...
const EXPENSE_CATS = ["Groceries","Bills & Utilities","Food & Dining","Transport","Fuel","Shopping","Entertainment","Health","Fitness","Personal Care","Rent","Home & Maintenance","Subscriptions","Insurance","Education","Travel","Pets","Childcare","Gifts & Donations","Miscellaneous"];
const INCOME_CATS = ["Salary","Freelance","Business","Rental Income","Interest & Dividends","Refunds & Cashback","Gifts","Other Income"];
const CAT_COLORS = ["#1E6B4E","#C97452","#C9A03D","#4C7A94","#8B6BB0","#3E8E7E","#B0574F","#6E8C4A","#A88B3F","#4E6E8C","#7A6E5E","#5C8A72","#B0894A","#6B5B95","#4F8FA6","#A65D57","#598C6E","#8C7548","#7E6BA6","#4E7A8C"];
...
function getViewMonth(){
  const saved = localStorage.getItem('budgetpro:viewMonth');
  const d = saved ? new Date(saved+'-01T00:00:00') : new Date();
  d.setDate(1);
  return d;
}
function setViewMonthState(d){ localStorage.setItem('budgetpro:viewMonth', monthKey(d)); }
...
async function dbSaveBudgets(newBudgets){
  ...
  if(upserts.length){
    const {error} = await sb.from('budgets').upsert(upserts, {onConflict:'user_id,category'});
    if(error){ console.error(error); throw error; }
  }
...
function displayName(user){
  ...
  return (full && full.trim()) ? full.trim() : (user.email ? user.email.split('@')[0] : 'there');
}
...
// Call at the top of every protected page's init(). Redirects to login.html
// if there's no signed-in user (and there IS a working Supabase connection).
async function requireAuth(){
  if(!sb) return null; // no DB configured yet — let the config banner explain instead
...
/* ===================== Export to Excel (one click, all data) ===================== */
function exportTransactionsToExcel(){
  ...
  if(typeof XLSX === 'undefined'){
    alert('The Excel export library failed to load. Check your internet connection and try again.');
    return;
  }
  ...
  XLSX.writeFile(wb, `BudgetPro-Transactions-${stamp}.xlsx`);
  showToast(`Exported ${rows.length} transaction${rows.length===1?'':'s'} to Excel`);
}
...
const PAGES = [
  ...
  {id:'chat', href:'chat.html', label:'Budget Chat', short:'Chat', icon:'<path d="M21 12c0 4.4-4 8-9 8-1.2 0-2.4-.2-3.4-.6L3 21l1.3-3.9C3.5 15.9 3 14 3 12c0-4.4 4-8 9-8s9 3.6 9 8z"/>'}
];

function buildShell(activePage, user){
  user = user || currentUser;
  const userBlock = user ? `
      <div class="user-menu">
        <div class="user-avatar">${initials(user)}</div>
        <div class="user-info">
          <div class="user-name">${firstName(user)}</div>
          <div class="user-email">${user.email||''}</div>
        </div>
        <button class="user-logout" onclick="logout()" aria-label="Log out" title="Log out">
  ...
  if(sidebar){
    sidebar.innerHTML = `
      <div class="brand">
        <div class="brand-mark">${LOGO_MARK}</div>
        <div class="brand-text"><div class="t1">Budget Pro</div><div class="t2">Plan. Track. Grow.</div></div>
      </div>
      <nav class="nav">
      ...
      <button class="export-btn" onclick="exportTransactionsToExcel()">
  ...
  if(topbar){
    topbar.innerHTML = `
      <div class="mt-brand">
        <div class="mt-mark">${LOGO_MARK}</div>
        <div><div class="t1">Budget Pro</div><div class="t2">Plan. Track. Grow.</div></div>
      </div>
      <div class="mt-actions">
        <button class="mt-export" onclick="exportTransactionsToExcel()" aria-label="Export to Excel"><svg viewBox="0 0 24 24"><path d="M12 3v12"/><polyline points="7 10 12 15 17 10"/><path d="M5 21h14"/></svg></button>
        <div class="mt-month">
          <button onclick="shiftMonth(-1)" aria-label="Previous month"><svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg></button>
          <span id="monthLabelMobile">—</span>
          <button onclick="shiftMonth(1)" aria-label="Next month"><svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
        ${user ? `<button class="mt-avatar" onclick="logout()" aria-label="Log out" title="Log out, ${firstName(user)}">${initials(user)}</button>` : ''}
      </div>`;
  }
  // Bottom nav — split into a left and right group so the [+] FAB can be
  // mathematically centered above the ribbon no matter how many items sit
  ...
  if(bottomNav){
    ...
    const left = [byId('dashboard'), byId('budgets')].map(link).join('');
    const right = [byId('transactions'), byId('insights'), byId('chat')].map(link).join('');
    bottomNav.innerHTML = `
      <div class="bn-group">${left}</div>
      <button class="bn-fab" onclick="handleAddClick()" aria-label="Add entry"><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>
      <div class="bn-group">${right}</div>`;
  }
  updateMonthLabels();
}
...
function renderTips(list){
  ...
  const spend = {};
  list.filter(t=>t.type==='expense').forEach(t=>{ spend[t.category]=(spend[t.category]||0)+t.amount; });
  // Pool of every tip relevant to this month's spending, plus the general tips —
  // shuffled fresh on every render, so the tips you see change each time you refresh.
  const spentCats = Object.entries(spend).sort((a,b)=>b[1]-a[1]).map(([c])=>c).filter(c=>TIP_LIBRARY[c]);
  const pool = [...spentCats.map(c=>TIP_LIBRARY[c]), ...GENERAL_TIPS];
  const tips = shuffleArr(pool).slice(0, Math.min(3, pool.length));
...
const CHAT_HISTORY_KEY = 'budgetpro:chatHistory';
let chatHistory = [];
...
function findCategory(lowerText){
  const all = [...EXPENSE_CATS, ...INCOME_CATS];
  return all.find(c=>lowerText.includes(c.toLowerCase())) ||
    all.find(c=>lowerText.includes(c.toLowerCase().split(' ')[0]));
}
function extractAmount(lowerText){
  const m = lowerText.replace(/,/g,'').match(/(\d+(\.\d+)?)/);
  return m ? parseFloat(m[1]) : null;
}
function monthStatusReply(list, totals){
  const rate = totals.income>0 ? Math.round((totals.net/totals.income)*100) : null;
  const totalBudget = EXPENSE_CATS.reduce((s,c)=>s+(budgets[c]||0),0);
  const remaining = totalBudget - totals.expense;
  let msg = `For ${monthLabelStr(viewMonth)}: you've earned ${fmt(totals.income)} and spent ${fmt(totals.expense)}, leaving ${fmt(Math.abs(totals.net))} ${totals.net<0?'in the red':'in savings'}.`;
  if(rate!==null) msg += ` That's roughly a ${rate}% savings rate.`;
  if(totalBudget>0) msg += ` You have ${fmt(Math.max(0,remaining))} left across your budgets${remaining<0?' — you\'re over overall':''}.`;
  else msg += ` You haven't set any budgets yet — ask me to "suggest a budget" and I'll put one together from your spending.`;
  return msg;
}
function whereCanISave(list){
  const spend = {};
  list.filter(t=>t.type==='expense').forEach(t=>{ spend[t.category]=(spend[t.category]||0)+t.amount; });
  const over = Object.keys(budgets).filter(c=> spend[c] && spend[c] > budgets[c]);
  const top = Object.entries(spend).sort((a,b)=>b[1]-a[1]).slice(0,2);
  let msg = '';
  if(over.length) msg += `You're over budget on ${over.join(' and ')}. `;
  if(top.length){
    msg += `Your biggest categories this month are ${top.map(([c,a])=>`${c} (${fmt(a)})`).join(' and ')}. `;
    const tip = TIP_LIBRARY[top[0][0]];
    if(tip) msg += tip;
  } else {
...
function suggestBudgetPlan(){
  const months = [0,1,2].map(i=> new Date(viewMonth.getFullYear(), viewMonth.getMonth()-i, 1));
  // Only count months that actually have transaction history, so a new user with
  // one month of data doesn't get a budget suggestion averaged down over 3 months.
  const monthsWithTx = months.filter(d=> txForMonth(d).length>0);
  const activeMonths = Math.max(1, monthsWithTx.length);
  const perCat = {};
  monthsWithTx.forEach(d=>{
    txForMonth(d).filter(t=>t.type==='expense').forEach(t=>{
      perCat[t.category] = (perCat[t.category]||0) + t.amount;
    });
  });
  const avgIncome = monthsWithTx.length
    ? monthsWithTx.reduce((s,d)=> s + computeTotals(txForMonth(d)).income, 0) / activeMonths
    : 0;
  const suggestion = {};
  EXPENSE_CATS.forEach(c=>{
  ...
  if(Object.keys(suggestion).length===0){
    lastSuggestion = null;
    return `I don't have enough transaction history yet to suggest budgets — log a few weeks of entries and ask me again.`;
  }
  lastSuggestion = suggestion;
  const lines = Object.entries(suggestion).map(([c,a])=> `${c}: ${fmt(a)}`).join(', ');
  const incomeNote = avgIncome>0 ? ` Based on your average income of ${fmt(Math.round(avgIncome))}, this leaves roughly a 20% savings rate.` : '';
  return `Here's a suggested monthly budget based on your recent spending (slightly below your 3-month average, to nudge you toward saving) — ${lines}.${incomeNote} Say "apply suggestion" if you'd like me to save these to your Budgets page.`;
}
async function handleChatIntent(raw){
  const text = raw.trim();
  const lower = text.toLowerCase();
  const list = txForMonth(viewMonth);
  const totals = computeTotals(list);

  if(/^(hi|hello|hey)\b/.test(lower)){
    return `Hi! I'm your Budget Pro assistant. Ask me things like "how am I doing this month?", "suggest a budget", "where can I save money?", or "set budget for Rent to 15000".`;
  }

  if(/apply/.test(lower) && lastSuggestion){
    try{
      const merged = {...budgets, ...lastSuggestion};
      await dbSaveBudgets(merged);
      budgets = merged;
      if(typeof onDataChanged === 'function') await onDataChanged();
      lastSuggestion = null;
      return `Applied — I've saved those suggested budgets to your Budgets page.`;
    }catch(e){
      return `Couldn't save the suggested budgets — check your Supabase setup and try again.`;
    }
  }
  ...
  if(/(set|update|change)\b.*budget/.test(lower) || /budget\b.*(to|at|=)\s*\d/.test(lower)){
    ...
    if(cat && EXPENSE_CATS.includes(cat) && amount){
      const newBudgets = {...budgets, [cat]: amount};
      try{
        await dbSaveBudgets(newBudgets);
        budgets = newBudgets;
        if(typeof onDataChanged === 'function') await onDataChanged();
        return `Done — I've set your ${cat} budget to ${fmt(amount)} for this month.`;
      }catch(e){
        return `I understood you want ${cat} budget at ${fmt(amount)}, but saving it failed. Check your Supabase connection and try again.`;
      }
    }
    return `Tell me the category and amount, like "set budget for Groceries to 6000".`;
  }

  if(/(suggest|plan|help).*(budget|save|savings)|budget.*(plan|suggest)/.test(lower)){
    return suggestBudgetPlan();
  }

  if(/(save money|cut|reduce|overspend|too much|spending less)/.test(lower)){
    return whereCanISave(list);
  }

  if(/(how am i doing|status|summary|overview|this month)/.test(lower)){
    return monthStatusReply(list, totals);
  }

  const cat = findCategory(lower);
  if(cat){
    const spent = list.filter(t=>t.category===cat).reduce((s,t)=>s+t.amount,0);
    const b = budgets[cat];
    if(/(spend|spent|spending|cost)/.test(lower) && b){
      return `You've spent ${fmt(spent)} on ${cat} this month, out of a ${fmt(b)} budget (${Math.round((spent/b)*100)}%).`;
    }
    return b
      ? `${cat}: ${fmt(spent)} logged this month, against a ${fmt(b)} budget.`
      : `${cat}: ${fmt(spent)} logged this month. You don't have a budget set for it yet — want me to set one? Just say e.g. "set budget for ${cat} to 5000".`;
  }

  return `I can help you plan your budget. Try asking: "how am I doing this month?", "where can I save money?", "suggest a budget", or "set budget for Groceries to 5000".`;
}
function renderChatWindow(){
  const win = document.getElementById('chatWindow');
  if(!win) return;
  if(chatHistory.length===0){
    win.innerHTML = `<div class="chat-empty">Ask me things like <em>"how am I doing this month?"</em> or <em>"set budget for Rent to 15000"</em>.</div>`;
    return;
  }
...
async function submitChat(){
  ...
  let reply;
  try{
    reply = await handleChatIntent(text);
  }catch(e){
    reply = `Something went wrong answering that — try rephrasing, or ask about your spending, budgets, or saving tips.`;
  }
  chatHistory[thinkingIndex].text = reply;
  saveChatHistory();
  renderChatWindow();
}
